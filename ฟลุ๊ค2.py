...
RfuBot=[lineMID]
Family=["u1d6a3463a3e12b760898a17823360b69",lineMID]
admin = "u1d6a3463a3e12b760898a17823360b69" 
...
settings = {
    ...
    "pname": {},
    "pro_name": {},
    "man1":"สนใจติดบอททุกชนิด ทักมาคับ",
    "man2":"สนใจติดบอททุกชนิด ทักมาคับ",
    "man3":"สนใจติดบอททุกชนิด ทักมาคับ",
    "message":"สนใจติดบอททุกชนิด ทักมาคับ",
    "comment":"สนใจติดบอททุกชนิด ทักมาคับ",
    "userAgent": [
...
╠★→Pict ▶ ลงดิสคนอืน
╠★→Cover ▶ ลงปกคนอืน
╠★→Video ▶ลงวีดีโอปกคนอืน
╠★→Copy ▶ ก็อปปกคนอืน
╠★→backup ▶ กับร่าง
╠★→Mimic on/off ▶พูดเลียนแบน
╠★→MimicList ▶ ดูคนทีพูดเลียนแบน
╠★→MimicAdd ▶ พูดเลียนแบน
╠★→MimicDel ▶ลบพูดเลียนแบน
...
╠★→Zt ▶แทคชื่อล่องหน
╠★→Zc ▶ ดูmidคนใส่ล่องหน
╠★→Zm ▶ ดู คท คนใส่ล่องหน
╠★→Mc
╠★→info ▶ ดูชื่อดูตัสดูmidดู คท ดูดิส
...
def listgrup():
    listGrup =  """╭════คำสั่งชุดตั้งค่า══════
╠★→Autojoin on/off → เขากลุ่ม
╠★→block on/off → ออโต้บล็อค
╠★→Autoleave on/off → แชทรวม
╠★→Autoread on/off → อ่านออโต้
╠★→Tag on/off → แทค
╠★→Selfbot on/off → หยุดบอท
╠★→Tag2 on/off → แทครูป
╠★→sticker on/off →ลิ้งสติ้กเก้อ
╠★→Cctv on/off → จับคนอ่าน
╠★→Mid on/off → เปิดปิดอ่านmid  
╠★→Share on/off →ส่งลิ้งโพส
╠════════════════
║                   ▶ตั้งข้อความ◀
╠════════════════
...
╠★→ต้อนรับ on/off ➠ ปิดทั้งเข้าทั้งออก
    sᴇʟғʙᴏᴛ
       ʙч'❥;иαиич so goođ★"""
    return listGrup
...
def lineBot(op):
    try:
        if op.type == 5:
                line.sendText(op.param1,"สวัสดีจ้า\nบริการติดตั้งบอททุกชนิด\nสนใจสอบถามรายละเอียดได้เลย")
...
#===============หาชื่อล่องหน===============================================================#
                    ...
                    if targets == []:
                        line.sendMessage(to, "🤔แน๊ะไม่มีคนใส่ล่องหนในกลุ่มนี้😂")
                    else:
                        mc = ""
                    ...
                    line.sendMessage(to, "เชลบอท\nʙч'❥;иαиич so goođ★\nกำลังเริ่มต้นใหม่ ... โปรดรอสักครู่ ..")
                    line.sendMessage(to, "บอทได้ทำการ เข้บาระบบใหม่อีกครั้งแล้ว....")
                    restartBot()
                elif text.lower() == 'ออน':
                    timeNow = time.time()
                    ...
                    try:
                        arr = []
                        owner = "u1d6a3463a3e12b760898a17823360b69"
                        creator = line.getContact(owner)
                        contact = line.getContact(lineMID)
                        grouplist = line.getGroupIdsJoined()
                        contactlist = line.getAllContactIds()
                        blockedlist = line.getBlockedContactIds()
                        ret_ = "╔══[ ☆ɢʜᴏsᴛ By.NANNY☆L ]"
...
#==============================================================================#
                ...
                elif text.lower() == 'ลิ้งแชร on':
                    settings["timeline"] = True
                    line.sendMessage(to, "ปิดลิ้งแชร์แล้ว.")
                elif text.lower() == 'ลิ้งแชร off':
                    settings["timeline"] = False
                    line.sendMessage(to, "ปิดลิ้งแชร์แล้ว.")
                elif text.lower() == 'autojoin on':
                    settings["autoJoin"] = True
                    line.sendMessage(to, "เปิดเขากลุ่มออโต้แล้ว.")
                elif text.lower() == 'autojoin off':
                    settings["autoJoin"] = False
                    line.sendMessage(to, "ปิดเขากลุ่มออโต้แล้ว.")
...
#==============================================================================#
                    ...
                    sendMessageWithMention(to, lineMID)
                    line.sendContact(to, "u1d6a3463a3e12b760898a17823360b69")
...
#==============================================================================#
                ...
                elif "ค้างเชิญ" in msg.text:
                    spl = msg.text.split("ลบเชิญ")
                    if spl[len(spl)-1] == "":
...
#==============================================================================#   
                    ...
                    line.sendMessage(msg.to,"เปิดแทคมาแทคกลับแล้ว.")
                
                elif msg.text in ["Respontag off","Tag off","My respon off","Respon:off"]:
                    settings["detectMention"] = False
                    line.sendMessage(msg.to,"ปิดแทคมาแทคกลับแล้ว.")
                         ...
                         if settings['detectMention'] == True:
                             ...
                             for mention in mentionees:
                                 ...
                                 #         sendMessageWithMention(to, contact.mid)
                                          line.sendMessage(msg.to, None, contentMetadata={"STKID":"35732342","STKPKGID":"3240911","STKVER":"1"}, contentType=7)
                                          break
										  
                if msg.text in ["sp","Sp"]:
                    line.sendText(msg.to,"▂ ▃ ▄ ▅ ▆ ▇ █ ....100.0%")
                if msg.text in ["5","55","555","5555","55555"]:
                    line.sendText(msg.to,"อารมณ์ดี๊ดี")
                if msg.text in ["กำ"]:
                    line.sendText(msg.to,"มุดนมหน่อย")				
